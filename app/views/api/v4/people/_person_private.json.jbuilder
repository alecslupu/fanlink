json.partial! "api/v4/people/person", locals: {person: person}
json.cache! ["v3", "private", person] do
  json.email person.email
  json.set! :product do
    json.internal_name person.product.internal_name
    json.id person.product.id
    json.name person.product.name
  end
end
if person.level_progresses.empty?
  json.level_progress nil
else
  json.level_progress person.level_progresses, partial: "api/v4/level_progresses/level_progress", as: :level
end

if person.person_rewards.empty?
  json.rewards nil
else
  json.rewards person.person_rewards, partial: "api/v4/person_rewards/person_reward", as: :reward
end
if person.blocked_people.empty?
  json.blocked_people nil
else
  json.blocked_people do
    json.array! person.blocked_people do |bp|
      json.id bp.id
      json.username bp.username
    end
  end
end
if person.admin? && person.portal_access.present?
  json.permissions person.portal_access.summarize
else
  json.permissions nil
end
