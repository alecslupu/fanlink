    ...
    module Courseware
      module Client
        class CertcoursesController < BaseController
          ...
          def index
            if Fanlink::Courseware::PersonCertificate.where(person_id: params[:person_id], certificate_id: params[:certificate_id]).present?
          ...
          def show
            ...
            certcourse_pages = Fanlink::Courseware::CoursePage.where(course_id: params[:id], content_type: 'quiz')
            if certcourse_pages.present?
              @quizzes = []
              certcourse_pages.each do |certcourse_page|
                quiz_page = Fanlink::Courseware::QuizPage.find_by(course_page_id: certcourse_page.id)

                next if quiz_page.blank?

                person_responses = Fanlink::Courseware::PersonQuizPageAnswer.where(person_id: params[:person_id], quiz_page_id: quiz_page.id)

                next if person_responses.blank?

                correct_answer = Fanlink::Courseware::QuizPageAnswer.find_by(quiz_page_id: quiz_page.id, is_correct: true)
                ...
                if person_correct_answer.present?
                  ...
                  is_correct = false
                  answer_text = failed_attempts.present? ? Fanlink::Courseware::QuizPageAnswer.find(failed_attempts.last.answer_id).description : nil
                end
                ...
                quiz = {
                  ...
                  course_pages_count: Fanlink::Courseware::Course.find(certcourse_page.course_id).course_pages_count,
                  page_order: certcourse_page.course_page_order,
