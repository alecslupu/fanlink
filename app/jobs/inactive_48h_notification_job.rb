class Inactive48hNotificationJob
  ...
  def perform
    ...
    ActsAsTenant.with_tenant(notification.product) do
      NotificationDeviceId.joins(:person).where(device_type: :ios).where("people.last_activity_at > ? AND people.last_activity_at < ?",Time.zone.now - 49.hour, Time.zone.now - 48.hour).select(:id).find_in_batches(batch_size: BATCH_SIZE) do |device_ids|
      ...
      NotificationDeviceId.joins(:person).where(device_type: :android).where("people.last_activity_at > ? AND people.last_activity_at < ?",Time.zone.now - 49.hour, Time.zone.now - 48.hour).select(:id).find_in_batches(batch_size: BATCH_SIZE) do |device_ids|
