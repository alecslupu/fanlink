FanlinkApi::API.endpoint <%= ":get_#{plural_name}" %> do
  method :get
  tag <%= singular_name %>
  path <%= "'/#{plural_name}'" %>
...
FanlinkApi::API.endpoint <%= ":get_a_#{singular_name}" %> do
  method :get
  tag <%= singular_name %>
...
FanlinkApi::API.endpoint <%= ":create_a_#{singular_name}" %> do
  method :post
  tag <%= singular_name %>
  path <%= "'/#{plural_name}'" %>
...
FanlinkApi::API.endpoint <%= ":update_a_#{singular_name}" %> do
  method :put
  tag <%= singular_name %>
  path <%= "'/#{plural_name}/{id}'" %> do
    id :int32
...
FanlinkApi::API.endpoint <%= ":destroy_a_#{singular_name}" %> do
  method :delete
  tag <%= singular_name %>
  path <%= "'/#{plural_name}/{id}'" %> do
    id :int32
