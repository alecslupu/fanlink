  ...
  describe '#create' do
    ...
    it 'should not sign up new user with an invalid email' do
      ...
      ActsAsTenant.with_tenant(product) do
        expect {
          post :create, params: {
            product: product.internal_name, person: { username: 'abc', email: 'anything', password: 'anything' }
          }
        }.to change { Person.count }.by(0)
        expect(response).to be_unprocessable
        expect(json['errors']).to include('Email is invalid.')
  ...
  describe '#index' do
    ...
    it 'should get all people with no filter' do
      ...
      ActsAsTenant.with_tenant(person.product) do
        ...
        people_list = [person, normal_person, person1, person2, person3, person4, person5]
