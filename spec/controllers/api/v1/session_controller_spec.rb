...
RSpec.describe Api::V1::SessionController, type: :controller do
  ...
  describe '#create' do
    it 'should log in a person with email from a regular account' do
      ...
      ActsAsTenant.with_tenant(person.product) do
        post :create, params: {
          email_or_username: person.email, password: 'badpassword', product: person.product.internal_name
        }
      end
      expect(response).to have_http_status(200)
    ...
    it 'should log in a person with username from a regular account' do
      ...
      ActsAsTenant.with_tenant(person.product) do
        post :create, params: {
          email_or_username: person.username, password: 'badpassword', product: person.product.internal_name
        }
      end
      expect(response).to have_http_status(200)
      expect(person_private_json(json['person'])).to be_truthy
    ...
    it 'should not log in a person with wrong username from a regular account' do
      ...
      ActsAsTenant.with_tenant(person.product) do
        post :create, params: {
          email_or_username: 'wrongusername', password: 'badpassword', product: person.product.internal_name
        }
      end
    ...
    it 'should not log in a person with wrong email from a regular account' do
      ...
      ActsAsTenant.with_tenant(person.product) do
        post :create, params: {
          email_or_username: 'wrongemail@example.com', password: 'badpassword', product: person.product.internal_name
        }
      end
      expect(response).to have_http_status(422)
    end
