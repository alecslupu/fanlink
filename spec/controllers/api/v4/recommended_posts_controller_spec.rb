...
RSpec.describe Api::V4::RecommendedPostsController, type: :controller do
  ...
  describe "GET index" do
    it "gets all recommended posts when not providing page param" do
    ...
    it "gets page 1 of recommended posts" do
      person = create(:person)
      ActsAsTenant.with_tenant(person.product) do
    ...
    it "gets page 2 of recommended posts" do
      person = create(:person)
      ActsAsTenant.with_tenant(person.product) do
        post_list = create_list(:recommended_post, 8, status: :published).reverse
        login_as(person)
    ...
    it "includes the polls" do
      person = create(:person)
      ActsAsTenant.with_tenant(person.product) do
        post_list = create_list(:recommended_post, 2, status: :published).reverse
        login_as(person)
        get :index
        expect(response).to be_successful
        posts = json["recommended_posts"].map { |p| p["id"].to_i }
