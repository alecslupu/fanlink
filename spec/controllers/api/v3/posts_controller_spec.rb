...
RSpec.describe Api::V3::PostsController, type: :controller do
  describe "#create" do
  ...
  describe "#index" do
    ...
    it "should get a list of posts for a date range with limit" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        ...
        expect(response).to be_successful
        expect(json["posts"].map { |p| p["id"] }).to eq([postloggedin.id, post22.id])
      end
    end
    ...
    it "should return correct language if device language provided" do
      skip "need to assess if this should work with V3 api"
    ...
    it "should get a list of posts for a person" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        ...
        expect(response).to be_successful
        expect(json["posts"].map { |p| p["id"] }).to eq([post12.id, post11.id])
      end
    end
  ...
  describe "#list" do
    ...
    it "should get list paginated at page 1" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        ...
        # expect(json["posts"].first).to eq(nil)
    ...
    it "should get the list of all posts filtered on person_id" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        ...
        expect(json["posts"].map { |jp| jp["id"] }.sort).to eq(posts.map { |p| p.id }.sort)
      end
    end
  ...
  describe "#share" do
    it "should get a post without authentication" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        ...
        get :share, params: { post_id: flinkpost.id, product: flinkpost.product.internal_name }
        expect(response).to be_successful
    ...
    it "should 404 with valid post in different product" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        get :share, params: { post_id: flinkpost.id, product: person.product.internal_name }
        expect(response).to be_not_found
      end
    end
    it "should 404 with invalid post id" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        login_as(person)
        flinkpost = create(:published_post, person: person)
        get :share, params: { product: person.product.internal_name, post_id: flinkpost.id + 1 }
        expect(response).to be_not_found
      end
    end
    it "should 422 with invalid product" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        ...
        get :share, params: { product: "thiscannotpossiblyexist", post_id: flinkpost.id }
        expect(response).to be_unprocessable
      end
    end
    ...
    it "should get a post for different product than logged in" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        login_as(person)
        flinkpost = create(:published_post, person: person)
        get :share, params: { product: flinkpost.product.internal_name, post_id: flinkpost.id }
