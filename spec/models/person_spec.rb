...
RSpec.describe Person, type: :model do
  ...
  context 'Associations' do
    describe '#has_many' do
      ...
      # courseware
      it { should have_many(:person_course_page_progress) }
      it { should have_many(:person_certificates) }
      it { should have_many(:certificates).through(:person_certificates) }
      it { should have_many(:person_courses) }
      it { should have_many(:courses).through(:person_courses) }
  ...
  describe '#send_course_attachment_email' do
    it 'sends a course on email' do
      ...
      course_page = create(:download_file_page).course_page
      expect {
        pc.person.send_course_attachment_email(course_page)
      }.to have_enqueued_job.on_queue('mailers').with(
        'PersonMailer', 'send_downloaded_file', 'deliver_now', {
          id: pc.person_id,
          certcourse_page_id: course_page.id
        }
      )
    end
  end
