  ...
  context 'Validations' do
    describe '#number_of_correct_answers' do
      describe 'adding more than one correct answer' do
        before(:each) do
          @available_question = create(:trivia_picture_available_question)
      ...
      describe 'adding no correct answer' do
        before(:each) do
          @available_question = create(:trivia_picture_available_question, with_answers: false)
          @available_question.available_answers << create_list(:trivia_picture_available_answer, 2, is_correct: false)
        end
        it 'does not save the question' do
          expect(@available_question.save).to be_falsey
        end
