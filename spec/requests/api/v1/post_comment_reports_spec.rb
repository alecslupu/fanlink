describe "PostCommentReports (v1)" do
  ...
  describe "#index" do
    ...
    it "should get all reports paginated to page 1" do
      ...
      expect(json["post_comment_reports"].first).to eq(post_comment_report_v1_json(@index_reports.last))
      expect(json["post_comment_reports"].last).to eq(post_comment_report_v1_json(@index_reports[-2]))
    end
    it "should get all reports paginated to page 2" do
      ...
      expect(response).to be_success
      expect(json["post_comment_reports"].count).to eq(2)
      expect(json["post_comment_reports"].first).to eq(post_comment_report_v1_json(@index_reports[-3]))
      expect(json["post_comment_reports"].last).to eq(post_comment_report_v1_json(@index_reports[-4]))
    end
