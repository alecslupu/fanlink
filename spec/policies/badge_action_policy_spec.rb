...
RSpec.describe BadgeActionPolicy, type: :policy do
  permission_list = {
    index: false,
    show: false,
    create: false,
  ...
  describe "defined policies" do
    subject { described_class.new(Person.new, BadgeAction.new) }
  ...
  context "logged out user" do
    subject { described_class.new(nil, BadgeAction.new) }
    ...
    describe "protected methods" do
      it do
        expect(subject.send(:module_name)).to eq("badge")
      end
      it do
  ...
  context "user with super admin role and with admin product" do
    ...
    subject { described_class.new(Person.new(role: :super_admin, product: product), BadgeAction.new) }
  ...
  context "Scope" do
    it "should return all the records" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        ...
        badge_action = create(:badge_action)
        badge_action2 = create(:badge_action)

        expect(BadgeAction.count).to eq(2)

        scope = Pundit.policy_scope!(person, BadgeAction.all)
        expect(scope.count).to eq(2)
        expect(scope).to include(badge_action)
        expect(scope).to include(badge_action2)
      end
    end
  end
end
