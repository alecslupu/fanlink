...
RSpec.describe NotificationDeviceIdPolicy, type: :policy do
  ...
  describe "defined policies" do
    subject { described_class.new(Person.new, NotificationDeviceId.new) }
  ...
  context "logged out user" do
    subject { described_class.new(nil, NotificationDeviceId.new) }
  ...
  context "user with super admin role and with admin product" do
    ...
    subject { described_class.new(Person.new(role: :super_admin, product: product), NotificationDeviceId.new) }
  ...
  context "Scope" do
    it "should return all the records" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        notification_device_id = create(:notification_device_id)
        notification_device_id2 = create(:notification_device_id)

        expect(NotificationDeviceId.count).to eq(2)

        scope = Pundit.policy_scope!(person, NotificationDeviceId.all)
        expect(scope.count).to eq(2)
        expect(scope).to include(notification_device_id)
        expect(scope).to include(notification_device_id2)
      end
    end
  end
end
