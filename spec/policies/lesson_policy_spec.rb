...
RSpec.describe LessonPolicy, type: :policy do
  ...
  describe "defined policies" do
    subject { described_class.new(Person.new, Lesson.new) }
  ...
  context "logged out user" do
    subject { described_class.new(nil, Lesson.new) }
  ...
  context "user with super admin role and with admin product" do
    ...
    subject { described_class.new(Person.new(role: :super_admin, product: product), Lesson.new) }
  ...
  context "Scope" do
    it "should return all the records" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        ...
        scope = Pundit.policy_scope!(person, Lesson.all)
        expect(scope.count).to eq(2)
        expect(scope).to include(lesson)
        expect(scope).to include(lesson2)
      end
    end
  end
end
