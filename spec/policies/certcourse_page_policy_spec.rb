...
RSpec.describe CertcoursePagePolicy, type: :policy do
  ...
  describe "defined policies" do
    subject { described_class.new(Person.new, CertcoursePage.new) }
  ...
  context "logged out user" do
    subject { described_class.new(nil, CertcoursePage.new) }
  ...
  context "user with super admin role and with admin product" do
    ...
    subject { described_class.new(Person.new(role: :super_admin, product: product), CertcoursePage.new) }
  ...
  context "Scope" do
    it "should return all the records" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        ...
        certcourse_page2 = create(:certcourse_page)

        expect(CertcoursePage.count).to eq(2)
