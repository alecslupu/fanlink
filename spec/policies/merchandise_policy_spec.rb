...
RSpec.describe MerchandisePolicy, type: :policy do
  ...
  describe "defined policies" do
    subject { described_class.new(Person.new, Merchandise.new) }
  ...
  context "logged out user" do
    subject { described_class.new(nil, Merchandise.new) }
    ...
    describe "protected methods" do
      it do
        expect(subject.send(:module_name)).to eq("merchandise")
      end
      it do
  ...
  context "user with super admin role and with admin product" do
    ...
    subject { described_class.new(Person.new(role: :super_admin, product: product), Merchandise.new) }
  ...
  context "Scope" do
    it "should return all the records" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        ...
        merchandise2 = create(:merchandise)

        expect(Merchandise.count).to eq(2)

        scope = Pundit.policy_scope!(person, Merchandise.all)
        expect(scope.count).to eq(2)
        expect(scope).to include(merchandise)
        expect(scope).to include(merchandise2)
      end
    end
  end
end
