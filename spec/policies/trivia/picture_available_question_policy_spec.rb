...
RSpec.describe Trivia::PictureAvailableQuestionPolicy, type: :policy do
  let(:master_class) { Trivia::PictureAvailableQuestion.new }
  permission_list = {
    index: false,
    show: false,
    create: false,
  ...
  context "Scope" do
    it "should only return the person quiz in current product" do
      ...
      post2 = ActsAsTenant.with_tenant(create(:product)) { create(:trivia_picture_available_question) }

      ActsAsTenant.with_tenant(person.product) do
        post = create(:trivia_picture_available_question)
        scope = Pundit.policy_scope!(person, Trivia::PictureAvailableQuestion)
        expect(scope.count).to eq(Trivia::PictureAvailableQuestion.where(product_id: person.product_id).count)
