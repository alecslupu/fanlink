...
RSpec.describe Trivia::HangmanQuestionPolicy, type: :policy do
  let(:master_class) { Trivia::HangmanQuestion.new }
  permission_list = {
    index: false,
    show: false,
    create: false,
  ...
  context "Scope" do
    it "should only return the person quiz in current product" do
      ...
      post2 = ActsAsTenant.with_tenant(create(:product)) { create(:trivia_hangman_question) }

      ActsAsTenant.with_tenant(person.product) do
        post = create(:trivia_hangman_question)
        scope = Pundit.policy_scope!(person, Trivia::HangmanQuestion)
