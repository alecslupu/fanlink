...
RSpec.describe Trivia::TopicPolicy, type: :policy do
  let(:master_class) { Trivia::Topic.new }
  permission_list = {
    index: false,
    show: false,
    create: false,
  ...
  context "Scope" do
    it "should only return the person quiz in current product" do
      ...
      post2 = ActsAsTenant.with_tenant(create(:product)) { create(:trivia_topic) }

      ActsAsTenant.with_tenant(person.product) do
        post = create(:trivia_topic)
        scope = Pundit.policy_scope!(person, Trivia::Topic)
        expect(scope.count).to eq(Trivia::Topic.where(product_id: person.product_id).count)
