...
RSpec.describe Trivia::HangmanAvailableQuestionPolicy, type: :policy do
  let(:master_class) { Trivia::HangmanAvailableQuestion.new }
  permission_list = {
    index: false,
    show: false,
    create: false,
  ...
  context "Scope" do
    it "should only return the person quiz in current product" do
      ...
      post2 = ActsAsTenant.with_tenant(create(:product)) { create(:trivia_hangman_available_question) }
