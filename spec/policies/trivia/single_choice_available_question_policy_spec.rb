...
RSpec.describe Trivia::SingleChoiceAvailableQuestionPolicy, type: :policy do
  let(:master_class) { Trivia::SingleChoiceAvailableQuestion.new }
  permission_list = {
    index: false,
    show: false,
    create: false,
  ...
  context "Scope" do
    it "should only return the person quiz in current product" do
      ...
      post2 = ActsAsTenant.with_tenant(create(:product)) { create(:trivia_single_choice_available_question) }

      ActsAsTenant.with_tenant(person.product) do
        post = create(:trivia_single_choice_available_question)
        scope = Pundit.policy_scope!(person, Trivia::SingleChoiceAvailableQuestion)
        expect(scope.count).to eq(Trivia::SingleChoiceAvailableQuestion.where(product_id: person.product_id).count)
