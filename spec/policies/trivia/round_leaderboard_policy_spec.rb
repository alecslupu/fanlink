...
RSpec.describe Trivia::RoundLeaderboardPolicy, type: :policy do
  let(:master_class) { Trivia::RoundLeaderboard.new }
  permission_list = {
    index: false,
    show: false,
    create: false,
  ...
  context "Scope" do
    it "should only return the person quiz in current product" do
      ...
      post2 = ActsAsTenant.with_tenant(create(:product)) { create(:trivia_round_leaderboard) }

      ActsAsTenant.with_tenant(person.product) do
        post = create(:trivia_round_leaderboard)
        scope = Pundit.policy_scope!(person, Trivia::RoundLeaderboard)
