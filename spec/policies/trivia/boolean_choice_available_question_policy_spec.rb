...
RSpec.describe Trivia::BooleanChoiceAvailableQuestionPolicy, type: :policy do
  let(:master_class) { Trivia::BooleanChoiceAvailableQuestion.new }
  permission_list = {
    index: false,
    show: false,
    create: false,
  ...
  context "Scope" do
    it "should only return the person quiz in current product" do
      ...
      post2 = ActsAsTenant.with_tenant(create(:product)) { create(:trivia_boolean_choice_available_question) }

      ActsAsTenant.with_tenant(person.product) do
        post = create(:trivia_boolean_choice_available_question)
        scope = Pundit.policy_scope!(person, Trivia::BooleanChoiceAvailableQuestion)
        expect(scope.count).to eq(Trivia::BooleanChoiceAvailableQuestion.where(product_id: person.product_id).count)
