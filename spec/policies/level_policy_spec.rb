...
RSpec.describe LevelPolicy, type: :policy do
  permission_list = {
    index: false,
    show: false,
    create: false,
  ...
  describe "defined policies" do
    subject { described_class.new(Person.new, Level.new) }
  ...
  context "logged out user" do
    subject { described_class.new(nil, Level.new) }
  ...
  context "user with super admin role and with admin product" do
    ...
    subject { described_class.new(Person.new(role: :super_admin, product: product), Level.new) }
  ...
  context "Scope" do
    it "should return all the records" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        ...
        expect(Level.count).to eq(2)

        scope = Pundit.policy_scope!(person, Level.all)
        expect(scope.count).to eq(2)
        expect(scope).to include(level)
        expect(scope).to include(level2)
      end
    end
  end
end
