...
RSpec.describe BadgeAwardPolicy, type: :policy do
  ...
  describe "defined policies" do
    subject { described_class.new(Person.new, BadgeAward.new) }
  ...
  context "logged out user" do
    subject { described_class.new(nil, BadgeAward.new) }
  ...
  context "user with super admin role and with admin product" do
    ...
    subject { described_class.new(Person.new(role: :super_admin, product: product), BadgeAward.new) }
  ...
  context "Scope" do
    it "should return all the records" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        ...
        badge_award = create(:badge_award)
        badge_award2 = create(:badge_award)

        expect(BadgeAward.count).to eq(2)

        scope = Pundit.policy_scope!(person, BadgeAward.all)
        expect(scope.count).to eq(2)
        expect(scope).to include(badge_award)
        expect(scope).to include(badge_award2)
      end
    end
  end
end
