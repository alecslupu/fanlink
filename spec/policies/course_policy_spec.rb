...
RSpec.describe CoursePolicy, type: :policy do
  ...
  describe "defined policies" do
    subject { described_class.new(Person.new, Course.new) }
  ...
  context "logged out user" do
    subject { described_class.new(nil, Course.new) }
  ...
  context "user with super admin role and with admin product" do
    ...
    subject { described_class.new(Person.new(role: :super_admin, product: product), Course.new) }
  ...
  context "Scope" do
    it "should return all the records" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        ...
        scope = Pundit.policy_scope!(person, Course.all)
        expect(scope.count).to eq(2)
        expect(scope).to include(course)
        expect(scope).to include(course2)
      end
    end
  end
end
