...
RSpec.describe InterestPolicy, type: :policy do
  ...
  describe "defined policies" do
    subject { described_class.new(Person.new, Interest.new) }
  ...
  context "logged out user" do
    subject { described_class.new(nil, Interest.new) }
    ...
    describe "protected methods" do
      it do
        expect(subject.send(:module_name)).to eq("interest")
      end
      it do
  ...
  context "user with super admin role and with admin product" do
    ...
    subject { described_class.new(Person.new(role: :super_admin, product: product), Interest.new) }
  ...
  context "Scope" do
    it "should return all the records" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        ...
        scope = Pundit.policy_scope!(person, Interest.all)
        expect(scope.count).to eq(2)
        expect(scope).to include(interest)
        expect(scope).to include(interest2)
      end
    end
  end
end
