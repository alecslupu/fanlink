...
RSpec.describe LevelProgressPolicy, type: :policy do
  ...
  describe "defined policies" do
    subject { described_class.new(Person.new, LevelProgress.new) }
  ...
  context "logged out user" do
    subject { described_class.new(nil, LevelProgress.new) }
  ...
  context "user with super admin role and with admin product" do
    ...
    subject { described_class.new(Person.new(role: :super_admin, product: product), LevelProgress.new) }
  ...
  context "Scope" do
    it "should return all the records" do
      ...
      ActsAsTenant.with_tenant(person.product) do
        level_progress = create(:level_progress)
        level_progress2 = create(:level_progress)

        expect(LevelProgress.count).to eq(2)

        scope = Pundit.policy_scope!(person, LevelProgress.all)
        expect(scope.count).to eq(2)
        expect(scope).to include(level_progress)
        expect(scope).to include(level_progress2)
      end
    end
  end
end
