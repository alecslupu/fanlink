module JsonHelpers
  module V3
    ...
    def following_v3_json(following, currnt_user)
      {
        "id"       => following.id.to_s,
        "follower" => person_v3_json(following.follower, currnt_user),
        "followed" => person_v3_json(following.followed, currnt_user)
      }
    end
    def level_v3_json(level, lang = nil)
      {
          "id"                  => level.id.to_s,
          "name"                => level.name,
    ...
    def merchandise_v3_json(merchandise)
      {
    ...
    def message_v3_json(msg)
      {
        ...
        "person" => person_v3_json(msg.person),
        "mentions" => message_mentions_v3_json(msg)
      }
    end
    def message_list_v3_json(msg)
      {
          "id"        => msg.id.to_s,
    ...
    def message_mentions_v3_json(msg)
    ...
    def message_reports_v3_json(msg_report)
      {
    ...
    def pending_badge_v3_json(earned, badge)
      {
        "badge_action_count"  => earned,
        "badge"               => badge_v3_json(badge)
      }
    end
    def person_private_v3_json(person, potential_follower = nil)
      person_v3_json(person, potential_follower).merge(
        "email" => person.email
      )
    end
    def person_v3_json(person, potential_follower = nil, lang = nil)
      following = (potential_follower) ? potential_follower.following_for_person(person) : nil
      {
        ...
        "role"              => person.role,
        "level"             => (person.level.nil?) ? nil : level_v3_json(person.level),
    ...
    def person_share_v3_json(person, potential_follower = nil, lang = nil)
      following = (potential_follower) ? potential_follower.following_for_person(person) : nil
      {
        "username"          => person.username,
        "picture_url"       => person.picture_url
    ...
    def post_comment_v3_json(post_comment)
      {
        "id"            => post_comment.id.to_s,
        "create_time"   => post_comment.created_at.to_s,
        "body"          => post_comment.body,
        "mentions"      => post_comment_mentions_v3_json(post_comment),
        "person"        => person_v3_json(post_comment.person)
      }
    end
    def post_comment_list_v3_json(post_comment, lang = nil)
      {
          "id"              => post_comment.id.to_s,
    ...
    def post_comment_mentions_v3_json(post_comment)
    ...
    def post_comment_report_v3_json(post_comment_report)
      {
    ...
    def post_v3_json(post, lang = nil, reaction = nil)
      {
        "id"          => post.id.to_s,
        ...
        "person" => person_v3_json(post.person),
        "post_reaction_counts" => post.reaction_breakdown.to_v3_json,
        "post_reaction"     => (reaction.nil?) ? nil : post_reaction_v3_json(reaction),
        "global"            => post.global,
    ...
    def post_share_v3_json(post, lang = nil, reaction = nil)
      {
        "body"        => (lang.present?) ? post.body(lang) : post.body,
        "picture_url" => post.picture_url,
        "person" => person_share_v3_json(post.person),
      }
    end
    def post_list_v3_json(post, lang = nil)
      {
        "id"              => post.id.to_s,
        "person"          => person_v3_json(post.person),
        "body"            => (lang.present?) ? post.body(lang) : post.body,
    ...
    def post_reaction_v3_json(post_reaction)
      {
    ...
    def post_report_v3_json(post_report)
      {
    ...
    def post_share_v3_json(post, lang = nil)
      {
        "body"        => (lang.present?) ? post.body(lang) : post.body,
        "picture_url" => post.picture_url,
    ...
    def relationship_v3_json(relationship, currnt_user)
      {
        "id"            => relationship.id.to_s,
        "status"        => relationship.status,
        "create_time"   => relationship.created_at.to_s,
        "update_time"   => relationship.updated_at.to_s,
        "requested_by"  => person_v3_json(relationship.requested_by, currnt_user),
        "requested_to"  => person_v3_json(relationship.requested_to, currnt_user)
      }
    end
  end
end
